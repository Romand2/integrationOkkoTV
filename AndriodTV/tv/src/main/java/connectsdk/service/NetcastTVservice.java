...
package connectsdk.service;
        ...
        import connectsdk.service.config.NetcastTVService;
        import connectsdk.service.config.ServiceConfig;
        import connectsdk.service.config.ServiceDisc;
        import connectsdk.service.netcast.NetcastAppNumberParser;
        import connectsdk.service.netcast.NetcastApplicationParser;
        import connectsdk.service.netcast.NetcastChannelParser;
        import connectsdk.service.netcast.NetcastHttpServer;
        import connectsdk.service.netcast.NetcastVirtualKeycodes;
        import connectsdk.service.netcast.NetcastVolumeParser;
...
public class NetcastTVservice extends DeviceService implements Launcher, MediaControl, MediaPlayer, TVControl, VolumeControl, ExternalInputControl, MouseControl, TextInputControl, PowerControl, KeyControl {
    ...
    List<URLServiceSub<?>> subscriptions;
    ...
    public NetcastTVservice(ServiceDisc serviceDescription, ServiceConfig serviceConfig) {
        ...
        subscriptions = new ArrayList<URLServiceSub<?>>();
    ...
    public void connect() {
        ...
        if (!(serviceConfig instanceof NetcastTVService)) {
            ServiceConfig.ServiceConfigListener serviceConfigListener = serviceConfig.getListener();
            serviceConfig = new NetcastTVService(serviceConfig.getServiceUUID());
            serviceConfig.setListener(serviceConfigListener);
        }

        if (DiscoveryManager.getInstance().getPairingLevel() == PairingLevel.ON) {
            if (((NetcastTVService) serviceConfig).getPairingKey() != null
                    && ((NetcastTVService)serviceConfig).getPairingKey().length() != 0) {

                sendPairingKey(((NetcastTVService) serviceConfig).getPairingKey());
            }
            else {
                showPairingKeyOnTV();
            }
...
//        mServiceReachability.start();
    ...
    public void sendPairingKey(final String pairingKey) {
        ...
        ResponseListener<Object> responseListener = new ResponseListener<Object>() {
            ...
            public void onSuccess(Object response) {
                ...
                ((NetcastTVService)serviceConfig).setPairingKey(pairingKey);
    ...
    @Override
    public ServiceSubscriptions<ChannelListener> subscribeCurrentChannel(final ChannelListener listener) {
        getCurrentChannel(listener);    // This is for the initial Current TV Channel Info.

        URLServiceSub<ChannelListener> request = new URLServiceSub<ChannelListener>(this, "ChannelChanged", null, null);
        request.setHttpMethod(ServiceCommand.TYPE_GET);
    ...
    public ServiceSubscriptions<State3DModeListener> subscribe3DEnabled(final State3DModeListener listener) {
        ...
        URLServiceSub<State3DModeListener> request = new URLServiceSub<State3DModeListener>(this, TARGET_3D_MODE, null, null);
        request.setHttpMethod(ServiceCommand.TYPE_GET);
        request.addListener(listener);
    ...
    @Override
    public ServiceSubscriptions<TextInputStatusListener> subscribeTextInputStatus(final TextInputStatusListener listener) {
        keyboardString = new StringBuilder();

        URLServiceSub<TextInputStatusListener> request = new URLServiceSub<TextInputStatusListener>(this, "KeyboardVisible", null, null);
        request.addListener(listener);
    ...
    private JSONArray parseApplicationsXmlToJSON(String data) {
        ...
        try {
            ...
            SAXParser saxParser = saxParserFactory.newSAXParser();
            NetcastApplicationParser handler = new NetcastApplicationParser();
            saxParser.parse(stream, handler);
    ...
    private void addSubscription(URLServiceSub<?> subscription) {
...
//    }
    ...
    public DLNAService getDLNAService() {
        if (dlnaService == null) {
            ...
            ConnectDevice device = discoveryManager.getAllDevices().get(serviceDescription.getIpAddress());
