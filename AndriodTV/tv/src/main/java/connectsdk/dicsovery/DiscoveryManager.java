        ...
        import connectsdk.DefaultPlatform;
        import connectsdk.core.Util;
        import connectsdk.device.ConnectDevice;
        import connectsdk.device.ConnectDeviceListener;
        import connectsdk.device.ConnectDevice;
        import connectsdk.device.DefaultConnectDevice;
        import connectsdk.service.DLNAService;
        import connectsdk.service.DeviceService;
        import connectsdk.service.DeviceService.PairingType;
        import connectsdk.service.NetcastTVservice;
        import connectsdk.service.command.ServiceCommandError;
        import connectsdk.service.config.ServiceConfig;
        import connectsdk.service.config.ServiceConfig.ServiceConfigListener;
        import connectsdk.service.config.ServiceDisc;


public class DiscoveryManager implements ConnectDeviceListener, DiscoveryProviderListener, ServiceConfigListener {
    ...
    Context context;
    ConnectDevice connectableDeviceStore;

    int rescanInterval = 10;

    private ConcurrentHashMap<String, ConnectDevice> allDevices;
    private ConcurrentHashMap<String, ConnectDevice> compatibleDevices;
     ...
     *
     ...
     @endcode
     */
    public static synchronized void init(Context context, ConnectDevice connectableDeviceStore) {
     ...
     * you should use DiscoveryManager.getInstance() instead.
     */
    public DiscoveryManager(Context context, ConnectDevice connectableDeviceStore) {
        this.context = context;
        this.connectableDeviceStore = connectableDeviceStore;

        allDevices = new ConcurrentHashMap<String, ConnectDevice>(8, 0.75f, 2);
        compatibleDevices = new ConcurrentHashMap<String, ConnectDevice>(8, 0.75f, 2);
                ...
                if (action.equals(WifiManager.NETWORK_STATE_CHANGED_ACTION)) {
                    ...
                    switch (networkInfo.getState()) {
                        ...
                        case DISCONNECTED:
                            ...
                            for (ConnectDevice device: compatibleDevices.values()) {
    ...
    public void setCapabilityFilters(List<CapabilityFilter> capabilityFilters) {
        ...
        for (ConnectDevice device: allDevices.values()) {
    ...
    public boolean deviceIsCompatible(ConnectDevice device) {
     ...
     */
    public void setConnectableDeviceStore(ConnectDevice connectableDeviceStore) {
        this.connectableDeviceStore = connectableDeviceStore;
    }
     ...
     * To disable the ConnectableDeviceStore capabilities of Connect SDK, set this value to nil. This may be done at the time of instantiation with `DiscoveryManager.init(context, null);`.
     */
    public ConnectDevice getConnectableDeviceStore() {
        return connectableDeviceStore;
    }

    // @cond INTERNAL
    public void handleDeviceAdd(ConnectDevice device) {
        if (!deviceIsCompatible(device))
            return;
    ...
    public void handleDeviceUpdate(ConnectDevice device) {
        if (deviceIsCompatible(device)) {
    ...
    public void handleDeviceLoss(ConnectDevice device) {
        for (DiscoveryManagerListener listenter: discoveryListeners) {
    ...
    public boolean isNetcast(ServiceDisc description) {
     ...
     */
    public Map<String, ConnectDevice> getAllDevices() {
        return allDevices;
    }
     ...
     */
    public Map<String, ConnectDevice> getCompatibleDevices() {
    ...
    public void onServiceConfigUpdate(ServiceConfig serviceConfig) {
        ...
        for (ConnectDevice device : getAllDevices().values()) {
    ...
    @Override
    public void onCapabilityUpdated(ConnectDevice device, List<String> added, List<String> removed) {
        handleDeviceUpdate(device);
    }

    @Override public void onConnectionFailed(ConnectDevice device, ServiceCommandError error) { }
    @Override public void onDeviceDisconnected(ConnectDevice device) { }
    @Override public void onDeviceReady(ConnectDevice device) { }
    @Override public void onPairingRequired(ConnectDevice device, DeviceService service, PairingType pairingType) { }
    ...
    public void onServiceAdded(DiscoveryProvider provider, ServiceDisc serviceDescription) {
        ...
        if (device == null) {
            device = new ConnectDevice(serviceDescription);
            device.setIpAddress(serviceDescription.getIpAddress());
            allDevices.put(serviceDescription.getIpAddress(), device);
            deviceIsNew = true;
        }
...
//        device.setLastSeenOnWifi(currentSSID);
    ...
    public void onServiceRemoved(DiscoveryProvider provider, ServiceDisc serviceDescription) {
        ...
        ConnectDevice device = allDevices.get(serviceDescription.getIpAddress());
    ...
    @SuppressWarnings("unchecked")
    public void addServiceDescriptionToDevice(ServiceDisc desc, ConnectDevice device) {
