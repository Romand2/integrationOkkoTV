 ...
 */
public class SimpleDevicePicker implements ConnectDeviceListener {
    ...
    protected ConnectDevice pendingDevice;

    // Connected, active device
    protected ConnectDevice activeDevice;
    ...
    public void showPicker() {
        ...
        pickerDialog = picker.getPickerDialog(activity.getString(selectDeviceResId), new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapter, View view, int pos,
                                    long id) {
                ConnectDevice device = (ConnectDevice) adapter.getItemAtPosition(pos);
     ...
     * @param device
     */
    public void selectDevice(ConnectDevice device) {
        if (device != null) {
    ...
    @Override
    public void onDeviceReady(final ConnectDevice device) {
    ...
    @Override
    public void onDeviceDisconnected(final ConnectDevice device) {
        if (device == pendingDevice) {
            pickFailed(device);
        }
    ...
    @Override
    public void onConnectionFailed(ConnectDevice device, ServiceCommandError error) {
        if (device == pendingDevice) {
            pickFailed(device);
        }
    ...
    @Override
    public void onPairingRequired(ConnectDevice device, DeviceService service, final PairingType pairingType) {
    ...
    protected void pickFailed(final ConnectDevice device) {
        Util.runOnUI(new Runnable() {
            @Override
            public void run() {
